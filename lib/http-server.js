...
function stopRequest(res, statusCode, message) {
...
function HttpServer(options) {
  ...
  const securityHeadersEnabled = options.securityHeaders === true || options.securityHeaders === 'true';
  if (securityHeadersEnabled) {
    if (typeof this.headers['X-Content-Type-Options'] === 'undefined') {
      this.headers['X-Content-Type-Options'] = 'nosniff';
    }
    if (typeof this.headers['X-Frame-Options'] === 'undefined') {
      this.headers['X-Frame-Options'] = 'DENY';
    }
    if (typeof this.headers['Referrer-Policy'] === 'undefined') {
      this.headers['Referrer-Policy'] = 'no-referrer';
    }
  }
  ...
  const maxUrlLength = Number.isFinite(Number(options.maxUrlLength))
    ? Math.max(1, Number(options.maxUrlLength))
    : 2048;
  const maxHeaderBytes = Number.isFinite(Number(options.maxHeaderBytes))
    ? Math.max(256, Number(options.maxHeaderBytes))
    : 8192;
  const maxRequestBodyBytes = Number.isFinite(Number(options.maxRequestBodyBytes))
  ...
  before.unshift(function (req, res) {
    ...
    const hostHeader = req.headers ? req.headers.host : undefined;

    if (requestUrl.includes('\0')) {
      stopRequest(res, 400, 'Malformed URL');
      return;
    }

    if (requestUrl.length > maxUrlLength) {
      stopRequest(res, 414, 'URI too long');
      return;
    }

    if (req.httpVersionMajor >= 1 && req.httpVersionMinor >= 1 && !hostHeader) {
      stopRequest(res, 400, 'Host header required');
      return;
    }

    if (typeof hostHeader === 'string') {
      if (hostHeader.includes(',') || /\s/.test(hostHeader)) {
        stopRequest(res, 400, 'Invalid Host header');
        return;
      }
      if (!/^[A-Za-z0-9.\-:\[\]]+$/.test(hostHeader)) {
        stopRequest(res, 400, 'Invalid Host header');
        return;
      }
    }

    if (req.headers && req.headers['content-length'] && req.headers['transfer-encoding']) {
      stopRequest(res, 400, 'Conflicting Content-Length and Transfer-Encoding');
      return;
    }
    ...
    if (req.headers && req.headers['content-length']) {
      ...
      if (!Number.isFinite(contentLength) || contentLength < 0) {
        stopRequest(res, 400, 'Invalid Content-Length');
        return;
      }
      if (contentLength > maxRequestBodyBytes) {
        stopRequest(res, 413, 'Payload too large');
        return;
      }
    }
    ...
    if (req.headers) {
      for (const [name, value] of Object.entries(req.headers)) {
        ...
        if (!/^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/.test(headerName)) {
          stopRequest(res, 400, 'Invalid header name');
          return;
        }
        if (/[\0\r\n]/.test(headerValue)) {
          stopRequest(res, 400, 'Invalid header value');
          return;
        }
        if (headerValue.length > maxHeaderBytes) {
          stopRequest(res, 431, 'Request header fields too large');
          return;
        }
      }
    }
    ...
    if (method === 'TRACE' || method === 'TRACK' || method === 'CONNECT') {
      stopRequest(res, 405, 'Method not allowed');
      return;
    }
    res.emit('next');
  });

  const accessLogEnabled = options.accessLog === true || options.accessLog === 'true';
  if (accessLogEnabled) {
    before.push(function (req, res) {
      ...
      res.once('finish', function () {
        ...
        console.log('[access]', remoteAddress, req.method, req.url, res.statusCode, durationMs + 'ms');
      });
      res.emit('next');
    });
  }
